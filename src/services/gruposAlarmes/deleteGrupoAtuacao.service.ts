...
const deleteGrupoAtuacaoService = async (id:number,gpAtuacaoId:number) => {
    ...
    const grupoAtuacao = await gruposAtuacaoRepository.findOneBy({id:gpAtuacaoId})
    if(!grupoAtuacao) throw new AppError(400,"Grupo não encontrado")

    const gruposAtuacaoAtualizados = grupoAlarme.gruposAtuacao.filter(grp => grp.id !== grupoAtuacao.id)
